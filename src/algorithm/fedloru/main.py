...
from src import MetricManager, init_weights, CosineAnnealingWarmupRestarts
from .lorumodel import LoRUConv2dModel
from .server import FedLoRUServer
from .client import FedLoRUClient

logger = logging.getLogger(__name__)

class FedLoRU():
    def __init__(self, args, model, server_dataset, client_datasets):
        super(FedLoRU, self).__init__()
    ...
    def train(self):
        ...
        self.server.global_model = copy.deepcopy(self._build_lowrank_model(
            basemodel=self.server.global_model, 
            keep_original_weights=False
        ))
        params_before = sum(p.numel() for p in self.model.parameters() if p.requires_grad)
        params_after = sum(p.numel() for p in self.server.global_model.parameters() if p.requires_grad)
        logger.info(f"Total trainable params after low-rank factorization: {params_after / 1_000_000:.2f}M")
        logger.info(f"Total trainable params before low-rank factorization: {params_before / 1_000_000:.2f}M")


        for curr_round in range(self.args.start_R, self.args.R + 1):
            ...
            self._request_global_aggregate(participating_clients, updated_sizes)
            
            if (curr_round % self.args.eval_every == 0) or (curr_round == self.args.R):
                self.evaluate(excluded_ids=participating_clients)

            if self.round in self.args.accumulate_and_reinit:
                self.server.global_model.merge_and_reinit() 
            elif curr_round in self.args.accumulate:
                self.server.global_model.merge()
    ...
    def _build_lowrank_model(self, basemodel, keep_original_weights):
        target_modules=["encoder", "layer1", "layer2", "layer3", "layer4"]
        return LoRUConv2dModel(
            ...
            lora_for_higher_rank=self.args.lora_for_higher_rank,
        )
    
    def _create_server(self):
        server = FedLoRUServer(args=self.args, server_dataset=self.server_dataset, model=self.model)
    ...
    def _create_clients(self):
        def __create_client(identifier, datasets):
            client = FedLoRUClient(args=self.args, training_set=datasets[0], test_set=datasets[-1], id=identifier)
            return client
    ...
    def _request_local_update(self, participating_client_ids):
        def __update_client(client, pbar, device=None):
            if self.args.dist:
                client.dist_update(pbar, device)
                return {client.id: len(client.training_set)}, {client.id: 1}
            else:
        ...
        for id in participating_client_ids:
            ...
            if client.model is None:
                client.download(self.server.global_model)
        ...
        with tqdm(
            ...
            desc="LR{:6} LORU{} -- ROUND {} Local Training".format(self.curr_lr, self.args.loru, self.round)
        ) as pbar:        
    ...
    def _request_global_aggregate(self, ids, updated_sizes):
        ...
        for key in global_model.state_dict().keys():
            ...
            if 'num_batches_tracked' in key:
                temp = self.clients[id].model.state_dict()[key]
            else:
                for id in ids:
                    temp += coefficients[id] * self.clients[id].model.state_dict()[key]
            global_model.state_dict()[key].data.copy_(temp)

        for id in ids:
            self.clients[id].model = None

        self.server.global_model = global_model
