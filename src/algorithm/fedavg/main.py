...
class FedAvg():
    ...
    def train(self):
        ...
        for curr_round in range(1, self.args.R + 1):
            ...
            self.server.global_model = self._request_global_aggregate(participating_clients, updated_sizes)
    ...
    def _create_server(self):
        server = FedAvgServer(args=self.args, server_dataset=self.server_dataset, model=self.model)
    ...
    def _create_clients(self):
        def __create_client(identifier, datasets):
            client = FedAvgClient(args=self.args, training_set=datasets[0], test_set=datasets[-1], id=identifier)
            return client
